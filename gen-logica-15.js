...
function costruisciImmagini() { 
 ...
 html += inputText('campo-img-dati','Immagine slide Dati (URL) — dimensioni ideali: 154x429px. Immagini di dimensioni diverse verranno ridimensionate automaticamente.','https://...'); 
...
function importaScheda() { 
 ...
 for (var i = 0; i < spans.length - 1; i++) { 
  if (spans[i].className === 'scheda-label') { 
   var lbl = spans[i].textContent.trim(); 
   if (lbl === 'Descrizione:'    && spans[i+1].className === 'scheda-entry') setTextarea('campo-aspetto',    spans[i+1]['inn'+'erHTML'].trim()); 
 ...
 // ── Immagini ─────────────────────────────────────────────── 
 // Header 
 var divLaterale = temp['querySelector']('.img-laterale'); 
