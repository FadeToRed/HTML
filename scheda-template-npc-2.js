...
function aggiornaHTMLSchedaNpc(d) {
 ...
 for (var i = 0; i < statCards.length; i++) {
  ...
  if (cn === 'stat-card' || cn === 'stat-card-vitale') {
   ...
   if (lbl && val) { var sid = mapStat[lbl.textContent.trim()]; if (sid) { val.textContent = sid==='vita'||sid==='aura' ? parseInt(d.stat[sid]).toLocaleString() : d.stat[sid]; } }
  }
 }
 ...
 if (potieriBox) {
  var nuovoPoteri = '';
  if (d.tecniche && d.tecniche.length > 0) { for(var ti=0;ti<d.tecniche.length;ti++) nuovoPoteri+='<span class="scheda-label">'+d.tecniche[ti].nome+':</span> <span class="scheda-entry">'+(d.tecniche[ti].desc||'—')+'</span>\n\n'; }
  else nuovoPoteri = '<span class="scheda-entry">—</span>';
...
function costruisciHTMLSchedaNpc(d, classeContenitore, classeOriginale, styleOriginale) {
 ...
 if (d.tecniche && d.tecniche.length > 0) { for(var ti=0;ti<d.tecniche.length;ti++) htmlNen+='<span class="scheda-label">'+d.tecniche[ti].nome+':</span> <span class="scheda-entry">'+(d.tecniche[ti].desc||'—')+'</span>\n\n'; }
 else htmlNen += '<span class="scheda-entry">—</span>';
 htmlNen += '</div>';
