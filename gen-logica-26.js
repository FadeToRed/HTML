...
function importaScheda() { 
 ...
 for (var i = 0; i < divs.length; i++) { 
  if (divs[i].className === 'poteri-box') { 
   ...
   // Box 1: Profili — se assente è Speed Duel
   var isSpeedDuel = !nenBoxList[1] || nenBoxList[1]['querySelector']('.nen-card') === null;
   // Rileva Speed Duel contando i box: SD ha 4 box (potere, t25, t50, t100), completo ne ha 5
   isSpeedDuel = (nenBoxList.length < 5);
   if (isSpeedDuel) {
    var rdSd = document['querySelector']('input[name="nen-modalita"][value="speedduel"]');
    if (rdSd) { rdSd.checked = true; aggiornaModoNen(); }
    // In SD: box1=t25, box2=t50(1), box3=t100
