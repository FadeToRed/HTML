...
function importaScheda() {
 ...
 for(var i=0;i<divs.length;i++) {
  if(divs[i].className==='dati-pg') {
   ...
   var statusValoriNpc = [];
   for(var p=0;p<parti.length;p++){var coppia=parti[p].trim().split(':');if(coppia.length>=2){var chiave=coppia[0].trim();var valore=coppia.slice(1).join(':').trim();if(chiave==='Classe')setSelect('campo-classe',valore);if(chiave==='Status')statusValoriNpc.push(valore);if(chiave==='Livello')setVal('campo-livello',valore);}}
   if(statusValoriNpc.length>0){setSelect('campo-status-0',statusValoriNpc[0]);for(var sv=1;sv<statusValoriNpc.length;sv++){aggiungiStatus();setSelect('campo-status-'+sv,statusValoriNpc[sv]);}}
   break;
  }
 }
